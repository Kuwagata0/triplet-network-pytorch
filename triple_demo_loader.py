...
class PHISH_t(data.Dataset):
    ...
    def _raw_check_exists(self):
        return os.path.exists(os.path.join(self.root, self.raw_folder, self.training_file)) and \
    ...
    def process(self):
        ...
        training_set = (
            read_edge_and_label_file(os.path.join(self.root, self.raw_folder)),
        )
        # test_set = (
    ...
    def make_triplet_list(self, ntriplets):
        ...
        for class_idx in range(10):
            # 对于节点标签而给边打标签，认为与钓鱼节点为源的标1，否则标0，从而选择边的相似应该如下
            # 对于标签分别为1，1，0的三个边 a, b, c,认为a,b应该首尾相连，而对c不做要求
            # 对于标签分别为1，0，0的三个边 a, b, c,则不做任何要求
...
def read_edge_and_label_file(path):
    edge_path = os.path.join(path, 'TransEdgelist.txt')
    label_path = os.path.join(path, 'label.txt')
    G = nx.MultiDiGraph()
    with open(edge_path, 'r') as f:
        for data in f.readlines():
            src, tgt, val, ts = data.split(',')
            G.add_edge(src, tgt, {'val': val, 'ts': ts})
        f.close()
    
    with open(label_path, 'r') as f:
        for data in f.readlines():
            node, label = data.split()
            G.nodes[node]['label'].add(label)
        f.close()
    return 
